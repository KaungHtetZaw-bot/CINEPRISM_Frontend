...
const Hero = ({ movie }: { movie: any }) => {
  ...
  return (
    <div className="relative h-screen w-full overflow-hidden bg-app transition-colors duration-500">
      {isLoading ? (
        <div className="w-full h-full bg-skeleton animate-pulse" />
      ) : (
        <div className="relative w-full h-full">
          <img
            ...
            alt="Hero Background"
          />
          <div className="absolute inset-0 bg-app/60 z-10 transition-colors" />
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6 z-20">
        <div className="max-w-3xl space-y-6 animate-in fade-in zoom-in duration-1000">
          ...
          <p className="text-accent font-black tracking-[0.4em] uppercase text-[10px] md:text-xs italic">
          ...
          <div className="flex flex-col md:flex-row items-center gap-3 w-full max-w-2xl mx-auto pt-6">
            <div className="relative w-full group">
              <input
                ...
                className="w-full px-6 py-5 rounded-sm bg-surface-1/50 backdrop-blur-xl border border-border text-main outline-none focus:border-accent transition-all text-lg placeholder:text-muted"
              />
              <div className="absolute bottom-0 left-0 h-0.5 bg-accent w-0 group-focus-within:w-full transition-all duration-500" />
            </div>

            <button
              onClick={goWithEmail}
              className="w-full md:w-auto whitespace-nowrap bg-main text-app md:px-12 px-5 md:py-5 py-3 rounded-sm font-black text-[11px] uppercase tracking-[0.2em] hover:bg-accent hover:text-black active:scale-95 transition-all flex items-center justify-center gap-2 shadow-2xl"
            >
              Get Started
