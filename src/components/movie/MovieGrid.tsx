      ...
      <section className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-7 gap-6">
        {movies.map((movie, index) => (
          <div key={`${movie.id}-${index}`} className="relative group mb-4">
            <button onClick={()=>removeFromRecent(movie.id,movie.type || movie.media_type)} className="absolute top-2 right-2 z-50 p-1.5 
             border border-white/10 text-white rounded-sm
             opacity-0 group-hover:opacity-100 
           hover:border-rose-500 hover:text-rose-500 hover:scale-110
             transition-all duration-300"><Trash size={14}/></button>
            <div onClick={()=>goToDetails(movie)}>
              <MovieCard movie={movie} />
            </div>
            <div className="md:mt-4 mt-2 px-1 space-y-1">
               <p className="text-sm font-black uppercase text-zinc-500 tracking-tighter truncate">
