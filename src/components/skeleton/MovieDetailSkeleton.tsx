    ...
    <div className="relative bg-app min-h-screen pb-24 overflow-hidden animate-pulse">
      ...
      <div className="relative h-[80vh] w-full bg-skeleton/50">
        ...
        <div className="absolute top-8 right-6 md:right-16 flex gap-3">
          {[1, 2].map((i) => (
            <div key={i} className="h-11 w-11 rounded-full border border-border bg-surface-1/50 backdrop-blur-md" />
          ))}
        </div>

        {/* Hero Content Skeletons */}
        <div className="absolute bottom-5 left-6 md:left-16 w-full max-w-5xl space-y-8 z-10">
          <div className="space-y-6">
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="flex gap-2">
                <div className="h-6 bg-surface-1 border border-border rounded-full w-20" />
                <div className="h-6 bg-surface-1 border border-border rounded-full w-20" />
              </div>
            </div>
          </div>
          ...
          <div className="flex gap-4">
            <div className="h-14 w-40 bg-accent/20 rounded-sm shadow-2xl" />
      ...
      <div className="px-6 md:px-16 mt-16 grid grid-cols-1 lg:grid-cols-12 gap-16">
        ...
        <div className="lg:col-span-8 space-y-20">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="h-5 bg-surface-1 w-full rounded-xs" />
              <div className="h-5 bg-surface-1 w-full rounded-xs" />
              <div className="h-5 bg-surface-1 w-[90%] rounded-xs" />
              <div className="h-5 bg-surface-1 w-[40%] rounded-xs" />
            </div>
          </div>
          ...
          <div className="space-y-10">
            <div className="flex justify-between items-end border-b border-border pb-4">
                ...
                <div className="h-2 bg-surface-1 w-20" />
            </div>
            <div className="flex gap-8 overflow-hidden">
              {[1, 2, 3, 4, 5].map((i) => (
                <div key={i} className="shrink-0 space-y-4">
                  <div className="h-48 w-36 bg-surface-1 rounded-sm border border-border" />
                  <div className="h-3 bg-skeleton w-28 rounded-xs" />
                  <div className="h-2 bg-surface-1 w-20 rounded-xs" />
                </div>
              ))}
            </div>
          </div>
        ...
        {/* 3. Sidebar Skeleton */}
        <div className="lg:col-span-4 bg-surface-1 p-10 rounded-sm border border-border h-fit space-y-10">
          {[1, 2, 3].map((group) => (
